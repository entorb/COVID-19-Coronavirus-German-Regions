...
parser.add_argument(
    "-p",
    "--place",
    help="Name of place to generate; by default, maps for select places are generated",
)
args = vars(parser.parse_args())

with open(
    (pathlib.Path() / "meta").with_suffix(".json"),
    newline="",
    encoding="utf-8",
) as file:
...
with open(
    (pathlib.Path() / "population").with_suffix(".json"),
    newline="",
    encoding="utf-8-sig",
) as file:
...
for place in places:
        ...
        if place == "UK":
            ...
            for k, v in data["utlas"].items():
                if k in main:
                    ...
                    unused += 1
                    if args["place"] is not None or unused < 9:
        ...
        try:
            for query in queries:
                ...
                with urllib.request.urlopen(url1) as response:
                    date = (
                        json.loads(response.read())["editingInfo"]["lastEditDate"]
                        / 1000
                    )
                ...
                url2 = (
                    url
                    + "query?where={}%3E0&outFields={},{},{}&returnGeometry=false&returnExceededLimitFeatures=true&f=pjson".format(
                ...
                with urllib.request.urlopen(url2) as response:
                    ...
                    for item in json.loads(response.read())["features"][start:]:
                        id = str(
                            item["attributes"][meta["query"][query][1][0]],
                        ).lstrip("0")
                        if id in main:
                            main[id]["cases"] = int(
                            ...
                            id not in [None, "None", "NA"]
                            and item["attributes"][meta["query"][query][1][1]]
                            is not None
                        ):
                            unused += 1
                            if args["place"] is not None or unused < 9:
    ...
    for area in main:
        if main[area]["population"] > 0:
            main[area]["pcapita"] = (
                main[area]["cases"] / main[area]["population"] * unit
            )
    ...
    with open(
        (pathlib.Path() / "template" / place).with_suffix(".svg"),
        newline="",
        encoding="utf-8",
    ) as file_in:
        with open(
            (pathlib.Path() / "results" / place).with_suffix(".svg"),
            "w",
            newline="",
            encoding="utf-8",
        ) as file_out:
                                ...
                                if i == 0:
                                    file_out.write(
                                        row.replace(
                                            ...
                                            "&lt; "
                                            + num.format(threshold[1]).replace(
                                                "_",
                                                "&#8201;",
                                            ),
                                        ),
    ...
    print(
        "{}: {} cases total in {} areas; {} figures unused".format(
            place,
            sum(cases),
            len(cases),
            unused,
